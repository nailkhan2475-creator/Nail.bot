...
// ---------------- CONFIG ----------------
const BOT_NAME_FREE = '𝐍𝐀𝐈𝐋 𝐂ʏʙᴇʀ 𝐁𝐎𝐓';
...
const config = {
  ...
  FREE_IMAGE: 'https://files.catbox.moe/vmnp79.png',
  NEWSLETTER_JID: '120363404278849242@newsletter', // replace with your own newsletter its the main newsletter
  ...
  SUPPORT_NEWSLETTER: {
    jid: '120363404278849242@newsletter',  // Your channel
  ...
  DEFAULT_NEWSLETTERS: [
    ...
    { 
      jid: '120363404278849242@newsletterr',  // Your channel
      emojis: ['❤️', '🌟', '🔥', '💯'],
      name: '𝐍𝐀𝐈𝐋 𝐂ʏʙᴇʀ 𝐁𝐎𝐓', //your channel name or just desplay name
      description: 'Free Channel'
    },
    // Other popular newsletters if u have more
    { 
      jid: '120363404278849242@newsletter', 
      emojis: ['🎵', '🎶', '📻'],
      name: 'Music Updates'
    }
...
const MONGO_DB = process.env.MONGO_DB || '𝐍𝐀𝐈𝐋 𝐂ʏʙᴇʀ 𝐁𝐎𝐓';
...
╭─「  \`🤖${title}\`  」 ─➤*  
*│
*│*🥷 *Oᴡɴᴇʀ :* ${config.OWNER_NAME || '𝐍𝐀𝐈𝐋 𝐂ʏʙᴇʀ'}
...
`.trim();
    ...
    const defaultImg = "https://files.catbox.moe/vmnp79.png";
...
case 'owner': {
  ...
  try {
    ...
    const title = userCfg.botName || '𝐍𝐀𝐈𝐋 𝐂ʏʙᴇʀ 𝐁𝐎𝐓';
...
case 'deleteme': {
  ...
  try {
    ...
    await socket.sendMessage(sender, {
      ...
      caption: formatMessage('*🗑️ SESSION DELETED*', '*✅ Your session has been successfully deleted from MongoDB and local storage.*', 𝐍𝐀𝐈𝐋 𝐂ʏʙᴇʀ 𝐁𝐎𝐓)
    }, { quoted: fakevcard });
...
process.on('uncaughtException', (err) => {
  ...
  try { exec(`pm2.restart ${process.env.PM2_NAME || '𝐍𝐀𝐈𝐋 𝐂ʏʙᴇʀ 𝐁𝐎𝐓'}`); } catch(e) { console.error('Failed to restart pm2:', e); }
});
